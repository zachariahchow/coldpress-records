...
const StoreItem = ({ productData, productOptions, artist }) => {
    ...
    const productOptionEls = productOptionsArr.map(opt => {
        ...
        return (
            <select name={opt.optionName} id={opt.optionName} className="block appearance-none w-1/2 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" key={productOptionsArr.indexOf(opt)}>
